...
class DataManager(object):
    ...
    def createImageFileList(self):
        self.imageFileList = [f for f in listdir(self.imageFolder) if isfile(join(self.imageFolder, f)) and 'mask_total' not in f]
    ...
    def createGTFileList(self):
        self.GTFileList = [f for f in listdir(self.GTFolder) if isfile(
            join(self.GTFolder, f)) and 'mask_total' in f and 'scan' not in f]
    ...
    def loadImages(self):
        ...
        for f in self.imageFileList:
            id = f.split('.')[0]
            filename = join(self.imageFolder, f)
            s = h5py.File(filename, 'r')
            cur_scan = s['scan'][:]
            self.sitkImages[id]=rescalFilt.Execute(sitk.Cast(sitk.GetImageFromArray(cur_scan), sitk.sitkFloat32))
    ...
    def loadGT(self):
        ...
        for f in self.GTFileList:
            id = f.split('.')[0]
            filename = join(self.GTFolder, f)
            m = h5py.File(filename, 'r')
            cur_mask = m['mask_total'][:]
            self.sitkGT[id] = sitk.Cast(sitk.GetImageFromArray(cur_mask), sitk.sitkFloat32)
